    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-sm md:text-base">Gestão de ativos educacionais e conteúdo programático.</p>
        </div>
        <button 
              ...
              <div key={course.id} className="group relative">
                ...
                >
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <ChevronRight size={20} className={selectedCourse?.id === course.id ? 'text-white' : 'text-slate-400'} />
                  </div>
                  <p className={`text-xs line-clamp-2 leading-relaxed ${selectedCourse?.id === course.id ? 'text-white/80' : 'text-slate-500'}`}>
                ...
                <div className="absolute top-3 right-3 flex gap-1 md:opacity-0 md:group-hover:opacity-100 transition-all">
                  ...
                  <button 
                    ...
                    className={`p-2 rounded-xl transition-colors ${selectedCourse?.id === course.id ? 'bg-white/10 text-white hover:bg-white/20' : 'bg-slate-50 border border-slate-200 text-rose-600 hover:bg-slate-100'}`}
                  >
                    <Trash2 size={14} />
                  </button>
                </div>
        ...
        <div className="lg:col-span-2">
          {selectedCourse ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-center p-8 md:p-16 border-2 border-dashed border-slate-200 rounded-[2.5rem] bg-slate-50/50 shadow-sm">
              <div className="p-6 md:p-8 bg-white rounded-full mb-6 md:mb-8 border border-slate-100 shadow-xl shadow-primary/5">
                <BookOpen size={32} className="text-primary animate-pulse md:w-12 md:h-12" />
              </div>
              <h3 className="text-xl md:text-2xl font-black text-slate-900 mb-3 tracking-tight">SELECIONE UM CURSO</h3>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between mb-6 md:mb-10">
                ...
                <button onClick={() => setIsCourseModalOpen(false)} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 transition-colors">
                  <X size={24} />
                </button>
              </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between mb-6 md:mb-10">
                ...
                <button onClick={() => setIsSubjectModalOpen(false)} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 transition-colors">
                  <X size={24} />
                </button>
              </div>
